...
int check(vector<int>&v, int win) {
	map<int, int>mp;
	int n = v.size();
	int sum = 0;
	for (int i = 0; i < win; i++) {
...
int bs(vector<int>&v, int l, int h, int &ans) {
	if (l > h)return 0;
	int mid = l + (h - l) / 2;
	if (check(v, mid))return 1;
	else return bs(v, l, mid - 1, ans);
}
int twoSubarrays(int n, vector<int> &v) {
	int l = 1, h = n;
	int ans = 0;
	return bs(v, l, h, ans);
}
